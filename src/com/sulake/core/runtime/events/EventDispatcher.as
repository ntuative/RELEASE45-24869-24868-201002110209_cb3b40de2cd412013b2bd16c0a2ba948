package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1001:uint = 1;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_718)
            {
               Core.crash("Error catched in EventDispatcher; some event listener crashed!",var_1068.errorID,var_1068);
            }
            return _result == const_465;
         }
         return false;
      }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_231[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1001) : uint(const_465);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_231)
            {
               _loc2_ = var_231[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_231[_loc1_];
            }
            var_231 = null;
