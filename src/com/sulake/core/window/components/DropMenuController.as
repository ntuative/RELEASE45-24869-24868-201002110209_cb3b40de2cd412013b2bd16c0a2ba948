package com.sulake.core.window.components
{
   ...
   public class DropMenuController extends InteractiveController implements IDropMenuWindow
   {
      ...
      private function getSubMenu() : DropMenuController
      {
         ...
         {
            var_128 = context.create(name + "::submenu","",WindowType.const_450,var_73,0 | 0,_loc1_,subMenuEventProc,null,0,null,[WindowController.EXCLUDE_CHILD_TAG]) as DropMenuController;
         }
         else
         {
      ...
      protected function populateMenuItemContainer(param1:Array, param2:DropMenuController, param3:Function) : void
      {
         ...
         while(_loc9_ < _loc5_)
         {
            _loc6_ = context.create(name + "::menu_item_" + _loc9_,param1[_loc9_],WindowType.const_428,var_73,0 | 0 | 0 | 0 | 0,null,param3,null,_loc9_,null,["_EXCLUDE"]) as DropMenuItemController;
         ...
         var _loc10_:IWindow = context.create(name + "::padding","",WindowType.const_288,var_73,0 | 0 | 0 | 0,new Rectangle(0,0,1,2),null,null,0,null,["_EXCLUDE"]);
         ...
         if(var_476 > -1 && _loc5_)
         {
            _loc4_.getListItemAt(var_476).setStateFlag(WindowState.const_29,true);
         }
      }
      ...
      protected function openMenuView() : void
      {
         if(!var_564)
         {
            if(open())
            {
               if(false)
               {
                  populateMenuItemContainer(var_475,getSubMenu(),subMenuEventProc);
                  var_564 = true;
               }
            }
         }
      ...
      protected function closeMenuView() : void
      {
         if(close())
         {
            ...
            var_564 = false;
            getTitleText().text = var_476 < var_475.length && var_476 > -1 ? "null" : caption;
         }
      }
   }
}
