package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_399 >= var_289.length)
         {
            return "";
         }
         var petColor:IPetColor = var_289[var_399] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_571[var_1501] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(var_399 < 0 || var_399 > var_289.length)
         {
            var_399 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_522.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_828);
               if(_loc11_)
               {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_522.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_399 >= 0 && var_399 < var_289.length)
         {
            _loc7_ = var_289[var_399] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_571[var_1501] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_828,_loc3_.id,_loc2_,AvatarScaleType.const_33);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_275,AvatarAction.const_507);
